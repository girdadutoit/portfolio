...
const AppSidebar = ({ role = 'tenant' }) => {
  ...
  return (
    <aside data-app-sidebar data-open={open ? 'true' : 'false'}
    aria-label={role === 'tenant' ? 'Tenant navigation' : 'Landlord navigation'} style={{
      ...
      <div data-sidebar-bar style={{ padding: '4px 8px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <Logo onClick={() => go('/')} size={20} />
        <button data-sidebar-burger aria-label={open ? 'Close menu' : 'Open menu'} aria-expanded={open}
        onClick={() => setOpen((v) => !v)}
        style={{
...
<div data-app-topbar style={{
  padding: '20px 36px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16,
  ...
  backdropFilter: 'blur(20px)', position: 'sticky', top: 0, zIndex: 10
    ...
    <div data-app-topbar-actions style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
      {children}
      <button aria-label="Notifications" style={{ width: 38, height: 38, borderRadius: 12, border: '1px solid var(--line)', background: 'white', display: 'grid', placeItems: 'center', position: 'relative', flexShrink: 0 }}>
  ...
  return (
    <div style={{ padding: '32px 36px', display: 'flex', flexDirection: 'column', gap: 28 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.3fr 1fr 1fr', gap: 20 }}>
        {/* Care Score card */}
        <div data-care-card className="solid-card" style={{ padding: 28, display: 'flex', alignItems: 'center', gap: 28 }}>
      ...
      <div>
        <div data-section-head style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 18, gap: 12 }}>
          <div>
          ...
          <button className="btn btn-coral btn-sm" onClick={() => go('/app/tenant/search')}>
  ...
  return (
    <div style={{ padding: '24px 36px' }}>
      ...
      <div className="solid-card" style={{ padding: 14, display: 'flex', gap: 10, alignItems: 'center', marginBottom: 20, flexWrap: 'wrap' }}>
        ...
        <div className="seg">
          <button className="on" aria-label="Grid view"><Icon name="grid" size={13} /></button>
          <button aria-label="Map view"><Icon name="map" size={13} /></button>
        </div>
...
const TenantApplications = () => {
  ...
  return (
    <div style={{ padding: '32px 36px' }}>
      ...
      <div className="solid-card" data-apps-table style={{ padding: '4px 8px' }}>
        <table className="tbl">
          <thead>
            <tr>
      ...
      <div data-apps-mobile style={{ display: 'none', flexDirection: 'column', gap: 12 }}>
        {apps.map((a, i) =>
        <button key={i} className="solid-card" style={{ padding: 16, textAlign: 'left', display: 'flex', flexDirection: 'column', gap: 12, cursor: 'pointer', width: '100%' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ width: 44, height: 44, borderRadius: 10, flexShrink: 0,
              background: `linear-gradient(135deg, oklch(0.82 0.10 ${i * 40}), oklch(0.88 0.08 ${i * 40 + 30}))` }} />
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 15, fontWeight: 600 }}>{a.prop}</div>
              <div style={{ fontSize: 12, color: 'var(--ink-3)' }}>{a.area} · {a.when}</div>
            </div>
            <div style={{ textAlign: 'right' }}>
              <div className="display" style={{ fontSize: 22, color: 'var(--coral-700)' }}>{a.score}</div>
              <div style={{ fontSize: 10, color: 'var(--ink-3)' }}>/100 MATCH</div>
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, flexWrap: 'wrap' }}>
            <span className={`chip ${statusColor[a.status]}`}>{a.status}</span>
            <div style={{ display: 'flex', gap: 3, alignItems: 'center' }}>
              {[0, 1, 2, 3].map((s) =>
              <div key={s} style={{ width: 24, height: 5, borderRadius: 99,
                background: s < a.step ? 'var(--coral-500)' : 'var(--line)' }} />
              )}
              <span style={{ fontSize: 11, color: 'var(--ink-3)', marginLeft: 6 }}>{['Applied', 'Reviewed', 'Shortlisted', 'Decision'][a.step - 1]}</span>
            </div>
          </div>
        </button>
        )}
...
const TenantProfile = () => {
  return (
    <div data-tenant-profile style={{ padding: '32px 36px', display: 'grid', gridTemplateColumns: '1fr 340px', gap: 28, alignItems: 'start' }}>
        ...
        <div className="solid-card" style={{ padding: 28 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div key={i}>
                ...
                <div data-lifestyle-row style={{ display: 'flex', gap: 6 }}>
                  {row.labels.map((l, j) =>
                <div key={l} aria-pressed={j === row.v} style={{
                  flex: 1, minHeight: 34, padding: '8px 6px', borderRadius: 10,
    ...
    <div data-app-layout style={{ display: 'flex', minHeight: '100vh' }}>
      ...
      <main style={{ flex: 1, minWidth: 0 }}>
        <AppTopbar title={titles[sub] || 'Dashboard'}>
          <button className="btn btn-coral btn-sm" data-app-topbar-cta style={{ width: "153px" }}>
...
<div data-messages style={{ padding: '32px 36px', display: 'grid', gridTemplateColumns: '320px 1fr', gap: 0, height: 'calc(100vh - 100px)' }}>
    <div data-messages-list className="solid-card" style={{ padding: 0, overflow: 'hidden' }}>
    ...
    <div data-messages-chat className="solid-card" style={{ marginLeft: 20, padding: 0, display: 'flex', flexDirection: 'column' }}>
